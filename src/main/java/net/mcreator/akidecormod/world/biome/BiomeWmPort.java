...
import net.mcreator.akidecormod.block.BlockWetGrass;
import net.mcreator.akidecormod.block.BlockTedToot;
import net.mcreator.akidecormod.block.BlockMangroveLog;
import net.mcreator.akidecormod.block.BlockAzaleaLeavesFlowering;
import net.mcreator.akidecormod.ElementsAkidecorMod;

import java.util.Random;

@ElementsAkidecorMod.ModElement.Tag
public class BiomeWmPort extends ElementsAkidecorMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Wm Port").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-16777012).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("wmport");
			topBlock = BlockWetGrass.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockWetGrass.block.getDefaultState().getBlock() || ground == Blocks.DIRT.getStateFromMeta(1).getBlock())
							&& (ground2 == BlockWetGrass.block.getDefaultState().getBlock()
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockAzaleaLeavesFlowering.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockAzaleaLeavesFlowering.block.getDefaultState());
										}
									}
								}
							}
								...
								for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										this.setBlockAndNotifyAdequately(world,
												...
												BlockTedToot.block.getDefaultState());
									}
								}
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockMangroveLog.block.getDefaultState().getBlock()
					|| blockType == BlockAzaleaLeavesFlowering.block.getDefaultState().getBlock()
					|| blockType == BlockWetGrass.block.getDefaultState().getBlock() || blockType == Blocks.DIRT.getStateFromMeta(1).getBlock();
		}
