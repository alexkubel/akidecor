...
import net.mcreator.akidecormod.block.BlockCherryLog;
import net.mcreator.akidecormod.block.BlockAzaleaLeavesFlowering;
import net.mcreator.akidecormod.block.BlockAzaleaLeaves;
import net.mcreator.akidecormod.ElementsAkidecorMod;

import java.util.Random;

@ElementsAkidecorMod.ModElement.Tag
public class BiomeNordalianLowland extends ElementsAkidecorMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Nordalian Lowland").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-14329397)
					.setHeightVariation(0.2F).setTemperature(0.5F));
			setRegistryName("nordalianlowland");
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockAzaleaLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockAzaleaLeaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.VINE.getDefaultState().getBlock()
									|| state.getBlock() == BlockAzaleaLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockCherryLog.block.getDefaultState());
								if (genh > 0) {
								...
								for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										this.setBlockAndNotifyAdequately(world,
												...
												BlockAzaleaLeavesFlowering.block.getDefaultState());
									}
								}
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockCherryLog.block.getDefaultState().getBlock()
					|| blockType == BlockAzaleaLeaves.block.getDefaultState().getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
