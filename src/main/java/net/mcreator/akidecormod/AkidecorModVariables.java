package net.mcreator.akidecormod;
...
public class AkidecorModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "akidecor_mapvars";
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				AkidecorMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				AkidecorMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "akidecor_worldvars";
		...
		@Override
		public void readFromNBT(NBTTagCompound nbt) {
			moners = nbt.getDouble("moners");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setDouble("moners", moners);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				AkidecorMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				AkidecorMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					AkidecorMod.PACKET_HANDLER.sendToAll(message);
				else
					AkidecorMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
